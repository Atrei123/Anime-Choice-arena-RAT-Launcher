...
def get_history_path():
    # Железобетонно берем папку, где лежит сам скрипт checker.py
    base_dir = os.path.dirname(os.path.abspath(__file__))
    return os.path.join(base_dir, HISTORY_FILE)

def load_history():
    h_path = get_history_path()
    
    if not os.path.exists(h_path):
        print(f"⚠️ Файл истории не найден по пути: {h_path}")
        return []
    ...
    try:
        ...
        print(f"❌ КРИТИЧЕСКАЯ ОШИБКА ЧТЕНИЯ history.json: {e}")
        print("Возможно файл пуст или сломался формат. Скрипт начнет с нуля.")
        # Если не хочешь, чтобы он качал заново при сломанном файле — можешь остановить скрипт тут
        return []
...
def analyze_game(filename, original_name):
    ...
    # 🔥 6. НОВАЯ ЛОГИКА: СБОР СТАТИСТИКИ 🔥
    ...
    for stat in stats_to_find:
        ...
        for pid_str, val_str in matches:
            try:
                slot = int(pid_str)
                val = int(val_str)
                # Умножаем урон и лечение на 1000
